...
function drawLine(ctx, p1, p2, color) {
    /**
     * Функция для отрисовки линии между двумя точками на canvas
     */
    ctx.beginPath();
...
function andrewsChainsMetod(points, ctx, delay) {

    /**
     * Функция осуществляет поиск минимальной выпуклой оболочки
     * по точкам рисунка и рендерит ее с задержкой, переданной
     * в параметре delay.
     */

    // Проверка наличия не менее 3 точек
    if (points.length < 3) return points;

    // Находим точку с минимальной x-координатой (левую нижнюю точку)
    let leftmost = 0;
    ...
    // Инициализация начальной точки и массива для хранения выпуклой оболочки
    let p = leftmost;
    let hull = [];

    // Начиная с самой левой нижней точки, движемся по часовой стрелке
    // добавляя точки к выпуклой оболочке, пока не вернемся к начальной точке
    let drawShell = () => {
            setTimeout(() => {
            drawLine(ctx, points[p], points[p], 'black');
            hull.push(points[p]);

            // Находим следующую точку оболочки
            let q = (p + 1) % points.length;
            for (let i = 0; i < points.length; i++) {
                // Если i-ая точка является более "левой" чем текущая q,
                // обновляем q
            ...
            // Теперь q становится текущей точкой
            p = q;

            if (p !== leftmost) {
                drawLine(ctx, hull[hull.length - 1], points[p], 'black');
                drawShell();
            } else {
                drawLine(ctx, hull[hull.length - 1], hull[0], 'black');
            }
        }, delay);
    };
...
andrewsChainsMetod(body, ctx, 1000);
